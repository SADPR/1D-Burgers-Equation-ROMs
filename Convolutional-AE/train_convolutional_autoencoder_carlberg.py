...
class ConvAutoencoder(nn.Module):
    def __init__(self, latent_dim=3):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(1, 8, kernel_size=27, stride=2, padding=12),  # (batch_size, 8, 256)
            nn.ELU(),
            nn.Conv1d(8, 16, kernel_size=26, stride=2, padding=12),  # (batch_size, 16, 128)
            nn.ELU(),
            nn.Conv1d(16, 32, kernel_size=28, stride=4, padding=12),  # (batch_size, 32, 32)
            nn.ELU(),
            nn.Conv1d(32, 64, kernel_size=28, stride=4, padding=12),  # (batch_size, 64, 8)
            nn.ELU(),
            nn.Conv1d(64, 64, kernel_size=4, stride=4, padding=0),  # (batch_size, 64, 2)
        )


        # Fully connected layers for latent space representation
        self.fc1 = nn.Linear(128, latent_dim)  # Compress to latent space
        self.fc2 = nn.Linear(latent_dim, 128)  # Expand back

        # Decoder
        self.decoder = nn.Sequential(
            nn.ConvTranspose1d(64, 64, kernel_size=4, stride=4, padding=0, output_padding=0),  # (batch_size, 64, 8)
            nn.ELU(),
            nn.ConvTranspose1d(64, 32, kernel_size=28, stride=4, padding=12, output_padding=0),  # (batch_size, 32, 32)
            nn.ELU(),
            nn.ConvTranspose1d(32, 16, kernel_size=28, stride=4, padding=12, output_padding=0),  # (batch_size, 16, 128)
            nn.ELU(),
            nn.ConvTranspose1d(16, 8, kernel_size=26, stride=2, padding=12, output_padding=0),  # (batch_size, 8, 256)
            nn.ELU(),
            nn.ConvTranspose1d(8, 1, kernel_size=27, stride=2, padding=12, output_padding=0),  # (batch_size, 1, 512)
        )
...
model = ConvAutoencoder(latent_dim=3)  # Specify the latent dimension
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=100, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            # Save the best model
            torch.save(model.state_dict(), 'best_conv_autoencoder_carlberg.pth')
        else:
            patience_counter += 1
...
torch.save(model, 'conv_autoencoder_complete_carlberg.pth')