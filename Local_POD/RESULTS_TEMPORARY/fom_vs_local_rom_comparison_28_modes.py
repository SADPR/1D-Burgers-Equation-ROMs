...
def plot_fom_rom_comparison():
    ...
    rom_filename_global_lspg = f"PROM_solution_28_modes.npy"
    ...
    plt.plot(X, U_ROM_local_lspg[:, time_indices[0]], color='blue', linestyle='-', linewidth=2, label='Local POD LSPG (20 Clusters, Avg 27.5 Modes)')
    for t_index in time_indices[1:]:
        plt.plot(X, U_ROM_local_lspg[:, t_index], color='blue', linestyle='-', linewidth=2)

    # Plot Global POD results (LSPG) (with label only once)
    plt.plot(X, U_ROM_global_lspg[:, time_indices[0]], color='red', linestyle='-', linewidth=2, label='Global POD LSPG (28 Modes)')
    for t_index in time_indices[1:]:
    ...
    # Annotate the plot
    plt.title(f'FOM vs ROM (LSPG) Comparison\nLocal POD (20 Clusters, Avg 27.5 Modes) vs Global POD (28 Modes)')
    plt.xlabel(r'$x$')
    plt.ylabel(r'$u$')
    plt.ylim(0, 7)
    plt.xlim(0, 100)
    ...
    plt.savefig(f"fom_local_vs_global_pod_comparison_28_modes.pdf", format='pdf')
