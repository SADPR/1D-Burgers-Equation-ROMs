...
def create_combined_gif(X, original_snapshot, pod_prom, pod_reconstructed, nTimeSteps, At, tol, num_modes):
    ...
    line_pod_reconstructed, = ax.plot(X, pod_reconstructed[:, 0], 'k-.', label=f'Local-POD Reconstructed (tol={tol}, modes={num_modes})')
    line_pod_prom, = ax.plot(X, pod_prom[:, 0], 'r--', label=f'Local-POD PROM (tol={tol}, modes={num_modes})')
    ...
    ani.save("local_pod_prom_vs_reconstruction.gif", writer=PillowWriter(fps=10))

    plt.show()

if __name__ == '__main__':
    snapshot_prom_pod = np.load("local_pod_prom.npy")
    snapshot_reconstruction_pod = np.load("local_pod_reconstruction.npy")
    original_snapshot = np.load("../training_data/simulation_mu1_4.76_mu2_0.0182.npy")
    ...
    print(f"Relative Error for Local-POD Reconstructed: {rel_error_pod_reconstructed:.4f}")
    print(f"Relative Error for Local-POD PROM: {rel_error_pod_prom:.4f}")
