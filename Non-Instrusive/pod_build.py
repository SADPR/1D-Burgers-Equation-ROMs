...
def save_modes(U, s, tolerances_squared, snapshots):
    ...
    for eps2, num_modes in zip(tolerances_squared, singular_values_taken):
        ...
        print(f'Saved modes for tolerance epsilon^2 = {eps2:.0e} -> modes/U_modes_tol_{eps2:.0e}.npy')

if __name__ == '__main__':
    data_dir = "../FEM/fem_training_data"
    # sort for deterministic stacking
    snapshot_files = sorted([f for f in os.listdir(data_dir) if f.endswith('.npy') and f.startswith('fem_simulation_')])
    all_snapshots = [np.load(os.path.join(data_dir, file)) for file in snapshot_files]
    all_snapshots = np.hstack(all_snapshots)  # [M, N]

    # Save the order for downstream scripts (training will parse mus in the same order)
    os.makedirs("modes", exist_ok=True)
    with open("modes/stack_order.json", "w") as f:
        json.dump({"files": snapshot_files}, f, indent=2)
