    ...
    def fom_burgers_2d(self, At, nTimeSteps, u0, mu1, E, mu2):
        ...
        for n in range(nTimeSteps):
            ...
            while (error_U > tol) and (k < max_iter):  # Nonlinear solver loop
                ...
                # C = self.compute_convection_matrix(U_n)

                C = self.compute_convection_matrix_SUPG(U_n)
                ...
                b[boundary_dofs_u_x] = mu1  # u_x = mu1 at x = 0
                
                # Solve the global system
                U_new_flat = spla.spsolve(A, b)
                ...
                error_U = np.linalg.norm(U_new - U_n) / np.linalg.norm(U_new)
                
                # Update the guess for the next iteration
                U_n = U_new.copy()
                U_n_flat = np.concatenate([U_n[:, 0], U_n[:, 1]])
                k += 1

            # Plot the current converged solution
            plot_3d_surface(U_n, self.X, self.Y)
            plot_2d_cuts(U_n, self.X, self.Y, line='x', fixed_value=50.0)
...
if __name__ == "__main__":
    # Domain and mesh
    a, b = 0, 100
    nx, ny = 100, 100  # Adjusted for quicker testing; increase for finer mesh
    ...
    # Time discretization
    Tf, At = 1.0, 0.05  # Adjusted for quicker testing; increase Tf for longer simulation
