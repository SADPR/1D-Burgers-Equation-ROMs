...
def plot_3d_surface_animation(U_FOM, U_POD_RBF, X, Y, mu1, mu2, num_modes, filename='3d_surface_animation.mp4', max_frames=500):
...
def plot_2d_cuts_animation(U_FOM, U_POD_RBF, X, Y, line, fixed_value, mu1, mu2, num_modes, filename, max_frames=500):
    ...
    filename = f"{filename}_mu1_{mu1:.3f}_mu2_{mu2:.4f}.mp4"
    ani.save(filename, writer='ffmpeg', fps=30)
    plt.close()
    print(f"Saved 2D cuts animation to {filename}")


def plot_2d_heatmap_animation(U_FOM, U_POD_RBF, X, Y, mu1, mu2, filename='heatmap_animation.mp4', max_frames=500):
    """Create an animation of side-by-side heatmaps of FOM and POD-RBF over time."""
    n_nodes = len(X)
    nTimeSteps = U_FOM.shape[1] - 1
    nTimeSteps = min(nTimeSteps, max_frames)
    time_step_size = 0.05
    ...
    vmin = min(U_FOM[:n_nodes, :].min(), U_POD_RBF[:n_nodes, :].min())
    vmax = max(U_FOM[:n_nodes, :].max(), U_POD_RBF[:n_nodes, :].max())
    ...
    def update_heatmap(n):
        ...
        im2 = ax2.pcolormesh(X_grid, Y_grid, U_POD_RBF_frames[n], cmap='viridis', shading='auto', vmin=vmin, vmax=vmax)
        ax1.set_title(f'FOM')
        ax2.set_title(f'POD-RBF 10+140 Modes')
        ax1.invert_yaxis()
        ax2.invert_yaxis()
        fig.suptitle(f'Time = {time:.2f} s', fontsize=16, y=0.95)
        return [im1, im2]
...
def plot_2d_cuts_overlay(U_FOM, U_POD_RBF, X, Y, line, fixed_value, mu1, mu2, num_modes, filename, time_steps, time_step_size=0.05):
    """Create static overlays of 2D cuts for FOM and POD-RBF at specified time steps."""
    n_nodes = len(X)
    ...
    if line == 'x':
        ...
        for t in time_steps:
            ...
            u_pod_rbf_values = U_POD_RBF[:n_nodes, index][indices]
            ax.plot(x_values, u_fom_values, 'k-', linewidth=2.5, label='FOM' if t == time_steps[0] else "")
            ax.plot(x_values, u_pod_rbf_values, 'g-', linewidth=2, label='POD-RBF 10+140 Modes' if t == time_steps[0] else "")
        ...
        for t in time_steps:
            ...
            ax.plot(y_values, u_pod_rbf_values, 'g-', linewidth=2, label='POD-RBF 10+140 Modes' if t == time_steps[0] else "")
    ...
    # This helps reduce the whitespace
    plt.tight_layout()

    # You can also adjust the subplots spacing if tight_layout doesn't help enough
...
def plot_2d_heatmap_overlay(U_FOM, U_POD_RBF, X, Y, mu1, mu2, num_modes, filename, time_steps, time_step_size=0.05):
    ...
    for i, t in enumerate(time_steps):
        ...
        U_POD_RBF_frame = U_POD_RBF[:n_nodes, index].reshape(len(y_values), len(x_values))
        
        axes[0, i].pcolormesh(X_grid, Y_grid, U_FOM_frame, cmap='viridis', shading='auto', vmin=vmin, vmax=vmax)
        axes[1, i].pcolormesh(X_grid, Y_grid, U_POD_RBF_frame, cmap='viridis', shading='auto', vmin=vmin, vmax=vmax)
        axes[0, i].set_title(f'FOM, t={t}')
        axes[1, i].set_title(f'POD-RBF 10+140 Modes, t={t}')
...
if __name__ == "__main__":
    ...
    plot_2d_cuts_overlay(U_FOM, U_POD_RBF, X, Y, 'x', fixed_value, mu1, mu2, num_modes, '2d_cuts_x_overlay', time_steps)
    plot_2d_cuts_overlay(U_FOM, U_POD_RBF, X, Y, 'y', fixed_value, mu1, mu2, num_modes, '2d_cuts_y_overlay', time_steps)
    # plot_2d_heatmap_animation(U_FOM, U_POD_RBF, X, Y, mu1, mu2)
    # plot_2d_heatmap_overlay(U_FOM, U_POD_RBF, X, Y, mu1, mu2, num_modes, 'heatmap_overlay', overlay_time_steps)
