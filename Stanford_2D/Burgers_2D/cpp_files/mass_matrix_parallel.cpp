...
Eigen::SparseMatrix<double> assemble_mass_matrix(py::array_t<double> X, py::array_t<double> Y, py::array_t<int> T,
    ...
    // Create a vector of triplet lists, one for each thread
    int n_threads = omp_get_max_threads();
    std::vector<std::vector<Eigen::Triplet<double>>> tripletLists(n_threads);
    for (int i = 0; i < n_threads; ++i) {
        tripletLists[i].reserve(2 * n_elements * n_local_nodes * n_local_nodes / n_threads);  // For u_x and u_y components
    }
    ...
    for (int elem = 0; elem < n_elements; ++elem) {
        ...
        auto &tripletList = tripletLists[thread_id];  // Local triplet list for this thread
    ...
    // Combine all local triplet lists into one global list
    std::vector<Eigen::Triplet<double>> globalTripletList;
    for (const auto &localTripletList : tripletLists) {
        globalTripletList.insert(globalTripletList.end(), localTripletList.begin(), localTripletList.end());
    }

    // Convert triplet list to sparse matrix
    Eigen::SparseMatrix<double> M_global(2 * n_nodes, 2 * n_nodes);
    M_global.setFromTriplets(globalTripletList.begin(), globalTripletList.end());

    return M_global;  // Return sparse matrix directly
}

// Binding the C++ function to Python using Pybind11
PYBIND11_MODULE(mass_matrix_parallel, m) {
