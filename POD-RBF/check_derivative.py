...
def manual_jacobian(q_p_train, W, q_p_sample, epsilon):
    ...
    for i in range(N):
        ...
        if np.abs(phi_r_i) > 1e-6:  # Ignore values that are too small
            ...
            jacobian += np.outer(W[i], dphi_dq_p.reshape(-1))  # Outer product to match (273, 28)
...
def gradient_check_pod_rbf(U_p, snapshot_column, q_p_train, W, epsilon_values, epsilon):
    ...
    for eps in epsilon_values:
        ...
        q_s_perturbed = interpolate_with_weights(q_p_train, W, q_p_perturbed.reshape(1, -1), epsilon)

        # Calculate the error term
        q_s_original = interpolate_with_weights(q_p_train, W, q_p.reshape(1, -1), epsilon)
