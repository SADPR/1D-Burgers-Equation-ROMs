...
# Custom mark_inset function for inset zoom boxes
...
def animate_fom_vs_pod_rbf(X, original_snapshot_fom, pod_rbf_reconstructed, nTimeSteps, At, zoom_window_size, fps=30):
    ...
    # Initial plot (black for FOM, red for POD-RBF)
    line_original_fom, = ax.plot(X, original_snapshot_fom[:, 0], 'k-', label='FOM Snapshot')  # Black for FOM
    line_pod_rbf, = ax.plot(X, pod_rbf_reconstructed[:, 0], 'r--', label=f'POD-RBF (28 primary modes, 301 secondary modes)')  # Red for POD-RBF
    ...
    axins.plot(X, pod_rbf_reconstructed[:, 0], 'r--')
    ...
    def update(frame):
        ...
        line_pod_rbf.set_ydata(pod_rbf_reconstructed[:, frame])
        ...
        axins.plot(X, pod_rbf_reconstructed[:, frame], 'r--')
    ...
    ani = FuncAnimation(fig, update, frames=nTimeSteps + 1, interval=1000 / fps)  # Set FPS to 30

    # Save animation as GIF with PillowWriter at 30 fps
    ani.save("fom_vs_pod_rbf_dynamic_zoom.gif", writer=PillowWriter(fps=fps))

    plt.close()

if __name__ == '__main__':
    # Load the reconstructed snapshots from .npy files
    pod_rbf_reconstructed = np.load('U_POD_RBF_PROM_solution.npy')[:, :-1]  # Remove last time step if needed
    ...
    animate_fom_vs_pod_rbf(X, original_snapshot_fom, pod_rbf_reconstructed, nTimeSteps, At, zoom_window_size)
