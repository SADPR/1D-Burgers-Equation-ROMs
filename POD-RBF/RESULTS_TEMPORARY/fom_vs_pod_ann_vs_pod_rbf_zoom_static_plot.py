...
def plot_fom_vs_pod_rbf_with_zoom():
    # Load domain
    a = 0
    b = 100
    m = int(256 * 2)
    ...
    # Load POD-RBF results
    pod_rbf_filename = "U_POD_RBF_PROM_solution.npy"
    U_POD_RBF_PROM = np.load(pod_rbf_filename)[:, :-1]  # Remove last timestep if necessary
    ...
    # Plot POD-RBF results with increased line width
    ax.plot(X, U_POD_RBF_PROM[:, time_indices[0]], color='red', linestyle='--', linewidth=3, label='POD-RBF (28 primary modes, 301 secondary modes)')
    for t_index in time_indices[1:]:
        ax.plot(X, U_POD_RBF_PROM[:, t_index], color='red', linestyle='--', linewidth=3)

    # Plot POD-ANN results with increased line width
    ax.plot(X, U_POD_ANN_PROM[:, time_indices[0]], color='blue', linestyle='-.', linewidth=3, label='POD-ANN (28 primary modes, 301 secondary modes)')
    for t_index in time_indices[1:]:
        ax.plot(X, U_POD_ANN_PROM[:, t_index], color='blue', linestyle='-.', linewidth=3)
    ...
    axins1.plot(X, U_POD_RBF_PROM[:, time_indices[0]], color='red', linestyle='--', linewidth=2.5)  # Increased line width in zoom
    axins1.plot(X, U_POD_ANN_PROM[:, time_indices[0]], color='blue', linestyle='-.', linewidth=2.5)  # Increased line width in zoom
    ...
    axins2.plot(X, U_POD_RBF_PROM[:, time_indices[2]], color='red', linestyle='--', linewidth=2.5)  # Increased line width in zoom
    axins2.plot(X, U_POD_ANN_PROM[:, time_indices[2]], color='blue', linestyle='-.', linewidth=2.5)  # Increased line width in zoom
    ...
    # Calculate and print L2 norm error for POD-RBF
    l2_error_pod_rbf = compute_l2_norm_error(U_FOM, U_POD_RBF_PROM)
    ...
    print(f"POD-RBF (28 primary modes, 301 secondary modes) L2 Error = {l2_error_pod_rbf:.6f}")

    # Annotate the plot
    ax.set_title(f'FOM vs PROM-LSPG with POD-RBF and POD-ANN (28 Primary Modes, 301 Secondary Modes)')
    ax.set_xlabel(r'$x$')
    ax.set_ylabel(r'$u$')
    ax.set_ylim(0, 10)
    ax.set_xlim(0, 100)
    ...
    plt.savefig("fom_vs_pod_rbf_ann_zoom_comparison_28_modes.pdf", format='pdf')
    plt.show()

# Run the comparison plot with zoom
plot_fom_vs_pod_rbf_with_zoom()
