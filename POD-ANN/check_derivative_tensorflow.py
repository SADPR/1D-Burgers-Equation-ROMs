...
# Function to perform gradient check
def gradient_check_pod_ann(U_p, snapshot_column, model, epsilon_values):
    ...
    # Record the gradients using GradientTape
    with tf.GradientTape() as tape:
        tape.watch(q_p_tensor)
        q_s = model(q_p_tensor)

    # Compute the gradient of q_s with respect to q_p_tensor
    g = tape.gradient(q_s, q_p_tensor).numpy().squeeze()
    ...
    for epsilon in epsilon_values:
        ...
        print(q_s_np)
        error = np.linalg.norm(q_s_perturbed - q_s_np - epsilon * (g @ v))
        errors.append(error)
...
epsilon_values = np.logspace(-9, -1, 10)  # Values of epsilon to test
