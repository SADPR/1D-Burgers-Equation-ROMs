...
def gradient_check_pod_ann(U_p, snapshot_column, model, epsilon_values):
    ...
    q_p_tensor.requires_grad_(True)

    # Pass q_p through the ANN to get the output q_s
    q_s = model(q_p_tensor)

    # Perform the backward pass to compute gradients
    q_s.backward(torch.ones_like(q_s))
    g = q_p_tensor.grad.detach().numpy().squeeze()
    ...
    for epsilon in epsilon_values:
        ...
        q_s_np = q_s.detach().numpy().squeeze()
